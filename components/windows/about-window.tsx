"use client"

import { useState } from "react"

export default function AboutWindow() {
  const [isFlipped, setIsFlipped] = useState(false)

  return (
    <div className="text-white scroll-container" style={{ maxHeight: "calc(80vh - 32px)" }}>
      {/* Profile Card with Flip Animation */}
      <div className="flex justify-center mb-6">
        <div
          className="relative w-32 h-32 cursor-pointer"
          onClick={() => setIsFlipped(!isFlipped)}
          style={{ perspective: "1000px" }}
        >
          <div
            className={`
              absolute inset-0 w-full h-full transition-transform duration-700 preserve-3d
              ${isFlipped ? "rotate-y-180" : ""}
            `}
            style={{ transformStyle: "preserve-3d" }}
          >
            {/* Front - Professional Photo */}
            <div className="absolute inset-0 w-full h-full backface-hidden rounded-full overflow-hidden border-4 border-white/30 shadow-lg card">
              <img
                src="/andrew-profile.jpg"
                alt="Andrew Sundaradhas"
                className="w-full h-full object-cover"
                style={{
                  objectPosition: "center 40%", // Adjusted to show full face including chin
                  transform: "scale(0.9)", // Reduced scale to fit more of your face in the circle
                }}
              />
            </div>

            {/* Back - Drone Icon */}
            <div
              className="absolute inset-0 w-full h-full backface-hidden rounded-full overflow-hidden border-4 border-white/30 shadow-lg rotate-y-180 card"
              style={{ transform: "rotateY(180deg)" }}
            >
              <div className="w-full h-full bg-gradient-to-br from-green-500 to-blue-600 flex items-center justify-center text-4xl">
                🚁
              </div>
            </div>
          </div>
        </div>
      </div>

      {/* Content with clean white typography */}
      <div className="text-center space-y-6">
        <h1 className="text-3xl font-bold text-white">Andrew Sundaradhas</h1>

        <div className="text-lg text-white font-bold">Context Engineer & Robotics Engineer</div>

        <p className="text-base text-white leading-relaxed font-normal">
          I specialize in Drone Tech and Machine Learning, building systems that actually work.
          <br />
          From autonomous drones to intelligent AI models — I make complex tech simple and effective.
          <br />
          If it flies, thinks, or needs to be smarter, I&apos;m your guy.
        </p>

        {/* Social Links */}
        <div className="flex justify-center space-x-4 mt-6">
          <a
            href="https://www.linkedin.com/in/andrew-sundaradhas/"
            target="_blank"
            rel="noopener noreferrer"
            className="social-link flex items-center space-x-2 bg-white/8 backdrop-blur-sm rounded-lg px-4 py-2 border border-white/10 group"
          >
            <svg className="w-5 h-5 text-white" viewBox="0 0 24 24" fill="currentColor">
              <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
            </svg>
            <span className="text-sm font-bold text-white">LinkedIn</span>
          </a>

          <a
            href="https://github.com/andrewsundaradhas"
            target="_blank"
            rel="noopener noreferrer"
            className="social-link flex items-center space-x-2 bg-white/8 backdrop-blur-sm rounded-lg px-4 py-2 border border-white/10 group"
          >
            <svg className="w-5 h-5 text-white" viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
            </svg>
            <span className="text-sm font-bold text-white">GitHub</span>
          </a>
        </div>
      </div>
    </div>
  )
}
